...
async function handleTranslate(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!record.summary && !record.tldr) {
      console.log(
        `[Translate API] Article ${record.id} has no AI content yet, forcing retry (503)`,
      );
      return NextResponse.json(
        {
          message: 'No AI content to translate yet, forcing retry',
          articleId: record.id,
        },
        { status: 503 },
      ); // Return 5xx to trigger Supabase Retry
    }
