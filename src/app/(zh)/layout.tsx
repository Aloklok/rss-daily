...
export default async function ZhRootLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <html lang="zh-CN" className={`${inter.variable} ${playfair.variable}`}>
          ...
          `}</style>
                ...
                <Providers>
                    <MainLayoutServer initialDates={dates} initialAvailableFilters={availableFilters} dict={zh}>
                        {children}
                    </MainLayoutServer>
                    <GlobalUI dict={zh} />
                </Providers>
