...
async function logFallbackError(headersList: Headers) {
  try {
    ...
    if (!path) {
      path = '(unknown_path)';
      // 识别 API 绕过或内部请求 (如 pg_net, 搜索引擎验证文件)
      if (userAgent.includes('pg_net')) {
        reason = 'Supabase内部请求(API绕过)';
      } else if (userAgent.includes('Mozilla') && !headersList.get('referer')) {
        reason = '静态资源缺失或爬虫尝试';
      } else {
        reason = '静态资源缺失或绕过';
      }
      ...
      debugMeta = {
        ...
        debug_vercel_id: headersList.get('x-vercel-id')?.split('/')[0] || 'none', // 仅保留段
