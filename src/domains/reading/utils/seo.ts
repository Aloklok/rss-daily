 ...
 * @param lang 'zh' | 'en'
 */
export function generateHighDensityDescription(
  date: string,
  allArticles: Article[],
  lang: 'zh' | 'en' = 'zh',
) {
  if (lang === 'en') {
    if (allArticles.length === 0)
      return `${date} Daily Briefing. Aggregated Tech News & RSS Highlights.`;
    ...
    if (allArticles.length > 3) {
      desc += `Including topics on ${topKeywords.join(', ')} and ${allArticles.length - 3} more curated tech updates.`;
    } else {
