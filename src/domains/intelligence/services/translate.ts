...
export async function translateBatchAndSave(
    ...
    try {
        const aiResult = await generateSiliconFlow(
            [
                { role: 'system', content: 'You are a professional software architect and technical translator. Always output valid JSON objects in the requested schema.' },
        ...
        const recordsToUpsert = translatedList.map(translated => {
            ...
            // 严格字段校验：确保关键内容字段不为空
            const requiredFields = ['title', 'summary', 'tldr', 'highlights', 'critiques', 'marketTake'];
            ...
            if (hasEmptyField) {
                console.warn(`[Translate] Skipping article ${original.id} due to empty fields in AI response.`);
                return null;
            }
            ...
            if (containsChinese) {
                console.warn(`[Translate] Skipping article ${original.id} because it contains Chinese characters.`);
                return null;
            }
