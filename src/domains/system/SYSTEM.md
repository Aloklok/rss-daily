...
为了集中管理核心业务逻辑与系统状态，系统内置了统一的管理员后台：

### 1. 权限守卫与布局
- **入口管理**：主站侧边栏 Logo 旁设置了微型配置齿轮，仅对具备 `isAdmin` 状态的用户可见。
- **布局约束 (`AdminLayout`)**：所有管理页面统一包裹在 `AdminLayout` 下，集成权限守卫逻辑。若非管理员访问，将自动拦截。
- **多语言适配**：后台管理界面与主站主题对齐（Stone/Orange 调性），并完美支持双语切换。

### 2. 核心功能模块
- **业务看板 (Dashboard)**：实时洞察全站内容分布、来源构成以及内容热词排行。
- **回溯中心 (Backfill)**：支持对历史文章进行批量的 AI 简报生成与重构。
- **系统设置**：提供前端直连的缓存手动刷新 (`/api/system/revalidate-date`) 出口。

### 3. 系统级重验证策略 (ISR Strategy)
针对 Vercel 免费计划进行的深度优化：
- **原子化更新**：不再采用周期性全站刷新，而是将 `revalidate` 时间设定为 1 年，完全转由数据库触发器 (`update briefing`) 驱动。
- **精准失效**：每次更新仅令受影响的日期缓存失效，极大程度保护了 Vercel 的服务端计算额度。
