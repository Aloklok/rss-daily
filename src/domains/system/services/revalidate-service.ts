...
export async function processRevalidation(
  ...
  try {
    ...
    if (lang === 'zh' && (article.summary || article.tldr)) {
      try {
        ...
        if (translationResult.success) {
          ...
          console.warn(
            `[RevalidateService] Translation failed for ${article.id}:`,
    ...
    if (!dateStr && lang === 'en' && article.id) {
      console.log(
        `[RevalidateService] Missing date for EN article ${article.id}, fetching from main table...`,
      );
      const { data } = await supabase
        .from('articles')
        .select('n8n_processing_date, published')
    ...
    if (dateStr) {
      ...
      if (
        lastRevalidated[throttleKey] &&
        now - lastRevalidated[throttleKey] < REVALIDATE_THROTTLE_MS
      ) {
        console.log(
      ...
      return { revalidated: true, date, clearedAvailableDates: true };
    }
