...
export function stripTags(html: string): string {
    if (!html) return '';
    // sanitize with empty ALLOWED_TAGS list removes all tags
    // KEEP_CONTENT: true (default) keeps the text content
...
export function sanitizeHtml(html: string): string {
    ...
    const clean = DOMPurify.sanitize(html, {
        ADD_TAGS: ['iframe'], // Optional: Add iframe if you need to support embeds (e.g. YouTube)
        ADD_ATTR: ['allow', 'allowfullscreen', 'frameborder', 'scrolling', 'target'] // Attributes for iframes/links
    });
    return clean;
}
