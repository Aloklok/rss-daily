 ...
 */
export async function generateBatchBriefing(candidates: BackfillCandidate[], modelId?: string): Promise<{ success: boolean; saved: number; total: number; titles?: string[]; errors: string[] }> {
    // 映射为 Action 接受的结构
    ...
    const result = (await generateBulkBriefingAction(mockArticles as unknown as Article[], modelId)) as BulkActionResult;
