...
- **午夜纸张模式 (Midnight Paper Mode)**: 精心调优的深色模式体验。
  - **沉浸式背景**: 采用深紫黑色 (`#191724`) 作为主背景，提供舒适、高端的阅读体验。
  - **排版升级**: 标题采用 **Playfair Display** 衬线体，正文采用 **Inter** 无衬线体，兼顾优雅与易读性。
  - **视觉一致性**: 所有深色颜色变量（如 `midnight-bg`, `midnight-selected`）均在 Tailwind 配置中统一管理，确保全站风格高度统一。
...
- **趋势工具集成**: 侧边栏提供“趋势”入口，采用极简主义设计，将工具分为“工具”、“趋势”和“数据库”三大类。
