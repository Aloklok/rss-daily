...
> | **按源浏览**      | `/sources`          | **Dynamic SSR** | 服务端 `fetch` + CSR  | ✅ 完全 (Shell) + 客户端数据 |
...
- **沉浸式简报头图**：
  ...
  - **内容类型过滤**: 新增 **"洞察 vs 新闻"** 维度。用户可一键筛选 "知识洞察型" (Deep Dive) 或 "新闻事件型" (News Event) 文章，满足不同阅读诉求。
- **午夜纸张模式**：
...
>
> 🧪 **测试指南**: 关于单元测试策略、命令及覆盖率报告，请参阅 **[TESTING.md](./docs/TESTING.md)**。
...
- `pnpm run test:coverage` - 运行测试覆盖率报告
