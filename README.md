...
- **双模访问控制**：
  ...
  - **管理员模式**：通过 URL Token (`?token=...`) 激活，拥有完整权限。
...
- **核心框架**: Next.js 15 (App Router), React, TypeScript
- **样式**: Tailwind CSS
- **状态管理**:
...
### 核心目录结构
- **`app/`**: 应用路由和页面。
  - **`layout.tsx`**: 根布局，包含全局 Providers 和 UI。
  - **`page.tsx`**: 首页（重定向逻辑）。
  - **`date/[date]/page.tsx`**: 每日简报页面 (SSR/ISR)。
  - **`article/[id]/page.tsx`**: 文章详情页面 (Static)。
  - **`trends/page.tsx`**: 趋势工具页面 (SSG)。
  - **`stream/[id]/page.tsx`**: 分类/标签流页面。
  - **`api/`**: Next.js Route Handlers (API 接口)。
- **`components/`**: UI 组件。
  - **`Sidebar.tsx`**: 侧边栏导航 (Client Component)。
  - **`ArticleCard.tsx`**: 文章卡片。
  - **`UnifiedArticleModal.tsx`**: 核心阅读模态框。
  - **`TrendsView.tsx`**: 趋势展示组件。
- **`lib/`**: 核心逻辑与工具。
