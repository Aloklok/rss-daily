...
const ArticleReaderView: React.FC<ArticleReaderViewProps> = ({ article, readerContent, isLoading, userTagLabels }) => {
    ...
    return (
        <article className="p-6 md:p-8 animate-fadeIn">
            ...
            <div id="article-reader-content" ref={contentRef} className="prose prose-lg dark:prose-invert max-w-none text-gray-800 dark:text-midnight-text-reader leading-relaxed select-text" dangerouslySetInnerHTML={{ __html: sanitizeHtml(removeEmptyParagraphs(readerContent.content)) }} />
        </article>
    );
};
