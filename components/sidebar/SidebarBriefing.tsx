...
const SidebarBriefing: React.FC<SidebarBriefingProps> = ({
    ...
    return (
        <div className="flex flex-col h-full space-y-4">
            {isInitialLoading ? (
                <div className="space-y-3 px-1 grow">{[...Array(5)].map((_, i) => <div key={i} className="h-12 bg-gray-100 dark:bg-midnight-card rounded-lg animate-pulse"></div>)}</div>
            ) : datesForMonth.length === 0 ? (
                <div className="flex flex-col items-center justify-center grow text-gray-400 dark:text-gray-500">
                ...
                <nav className="flex flex-col gap-2 grow overflow-y-auto pr-1 pb-4">
            ...
            <div className="mt-auto pt-2">
                <div className="relative group">
                    <select
                        ...
                        className="w-full appearance-none bg-transparent border-none text-gray-800 py-2 pl-3 pr-8 rounded-md focus:outline-hidden cursor-pointer absolute inset-0 z-10 opacity-0"
                        aria-label="选择月份"
                        title="选择月份"
                    >
                        {allDisplayMonths.map(month => (<option key={month} value={month}>{formatMonthForDisplay(month)}</option>))}
                    </select>
                    <div className="w-full flex items-center justify-between bg-white dark:bg-midnight-card border border-gray-200 dark:border-midnight-border shadow-xs group-hover:border-indigo-300 dark:group-hover:border-indigo-700 transition-all duration-200 text-gray-700 dark:text-gray-200 py-2.5 px-4 rounded-lg pointer-events-none">
