...
- **自动化与运维**:
  - **全链路触达**: 向量生成已集成至 `scripts` (存量)、`gemini.ts` (增量) 和 `webhook` (自动补全)，确保数据始终新鲜。
  - **维护工具**: 提供 `npm run search:backfill` 和 `npm run search:update`。
...
| 数据类型             | 来源         | 缓存策略               | 说明                                                          |
| :------------------- | :----------- | :--------------------- | :------------------------------------------------------------ |
| **日期列表** (Dates) | Supabase RPC | 请求级 Memoization     | 同一次 SSR/ISR 渲染内去重 RPC；跨请求不强缓存，跨天可即时更新 |
