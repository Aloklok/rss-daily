...
- **缓存周期**: 7 天 ISR
- **重验机制**: 每次用户操作(收藏/已读)都会触发 `revalidate-date` 清除该日期缓存（仅为了更新文章内容，而非用户状态）
- **状态分离**: **SSR 层不再合并 FreshRSS 状态** (Scheme C)
- **客户端补齐**: **BriefingClient 加载后通过 `useArticleStateHydration` 异步并发获取用户已读/星标状态**
- **首屏效果**: 极速 HTML (SEO 友好)，用户状态在 0.5s 后渐入显示
- **一致性保证**: 状态与缓存解耦，爬虫永远获取 <200ms 的纯净 HTML

```
用户点击收藏 → 前端状态立即更新 → 触发 revalidate-date（可选）
下次访问 → SSR 直出静态文章内容 (TTFB < 200ms) → 客户端 JS 并发请求 FreshRSS → 图标亮起
```
...
针对根目录不同页面的特性，项目采用了差异化的渲染策略：

| 页面                  | 渲染模式          | 决策原因                                                                         |
| :-------------------- | :---------------- | :------------------------------------------------------------------------------- |
| **首页 (/)**          | **Force Dynamic** | 1. 根据当前时间自动预选“时段”；2. 服务端直接处理 `searchParams` 以支持搜索直出。 |
| **日期页 (/date)**    | **ISR (7 Days)**  | 日期固定，不随访问时间变化，最适合静态化以实现极速加载。                         |
| **源列表 (/sources)** | **Force Dynamic** | 依赖外部 FreshRSS API 凭据，为规避构建时预渲染失败风险，维持动态模式。           |
