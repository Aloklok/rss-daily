...
| :-------------- | :-------------- | :----------- | :--------------- | :--------------------- |
| **首页 / 简报** | `/`             | **ISR (7d)** | Webhook 精准清除 | **客户端异步获取状态** |
| **归档中心**    | `/archive`      | **SSR**      | 每日更新         | **直出静态链接**       |
| **日期简报**    | `/date/[date]`  | **ISR (7d)** | Webhook 精准清除 | **客户端异步获取状态** |
...
- **客户端补齐**: 页面加载后通过 `useArticleStateHydration` 异步获取状态
...
```
文章内容变更 → Webhook 判断日期 → 清除对应缓存 → 下次访问重新生成
用户状态变更 → 仅更新 FreshRSS → 客户端下次访问时自动获取最新状态
```
