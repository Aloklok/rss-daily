...
>
> **Edge 路由策略 (Cloudflare Rule)**:
>
> - **逻辑**: 在 Cloudflare 层直接拦截首页 (`/`) 流量。若源 IP 非中国 (`ne CN`) 且无 `NEXT_LOCALE` Cookie，则重定向至 `/en`。
> - **Bot 策略**: Googlebot (US IP) 会被此规则重定向到 `/en`，与百度 (CN IP) 访问 `/` 形成自然分流。
> - **Cookie 策略**: 前端 `LanguageSwitcher` 组件会在切换时写入 `NEXT_LOCALE` Cookie (Max-Age 1y)，Cloudflare 规则会自动放行持有此 Cookie 的用户。
