...
- **`briefings/`**: 简报数据服务。
  ...
  - `POST /api/podcasts/generate`: **[异步化架构]** 按需生成播客音频。采用原子化 `upsert` 解决 DB 约束冲突，并利用 `after()` API 确保后台任务在 Vercel 生产环境下 100% 跑完。返回 `{ script, audioUrl, status: 'processing' }`。
  - `GET /api/podcasts/fetch`: 获取已存在的播客文稿与记录。支持 **哈希指纹审计**，若检测到音频版本过旧则返回 `status: 'stale'` 并清空 `audioUrl`，驱动系统执行静默自愈。
- **`meta/`**: 元数据服务。
