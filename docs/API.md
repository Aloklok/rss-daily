...
- **`briefings/`**: 简报数据服务。
  ...
  - `POST /api/podcasts/generate`: **[优化]** 按需生成播客音频。集成 **Hash-based 幂等校验**，通过对文稿执行 MD5 摘要，优先从 Storage 召回已存在的音频。支持 `language` (zh/en), `modelId` 与 `enableThinking` 参数。英文音频文件名会自动附带 `_en` 后缀以实现物理隔离。
