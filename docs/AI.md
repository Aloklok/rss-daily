...
- **模型锁定策略 (Version Locking)**：2026 年起，Google 调整了 `latest` 别名的指向（通常指向配额极紧的 2.5 系列）。为保证稳定性，系统已**禁用自动升级/纠偏逻辑**，强制锁定使用 `gemini-1.5-flash`、`gemini-2.5-flash-lite` 等具体的稳定版本 ID。
- **“羊毛”配额池 (Quota Buffering)**：对话框型号选择器集成了多个具备独立配额权重的模型（如 1.5 Robotics 预览版、2.5 Flash Lite 等）。当主模型触发 429 报错时，管理员可通过切换“独立池子”型号实现每日可用次数的最大化。
